...
function Map() {
    const [X, setX] = useState("53.08861485884185");
    const [Y, setY] = useState("33.38702505613743");
    ...
    useEffect(() => {
        ...
        if (detectedObjects.length != 0) {
            setX(detectedObjects[0].x)
            setY(detectedObjects[0].y)
        }


        map = L.map('mapid').setView([parseFloat(X), parseFloat(Y)], 12);
            ...
            detectedObjects.forEach(detectedObject => {
                if (detectedObject.missionId == null && detectedObject.isDesired === false) {
                    return L.marker([parseFloat(detectedObject.x), parseFloat(detectedObject.y)], {
                        icon: FreeIcon,
                    }).addTo(map).bindPopup(ReactDOMServer.renderToString(<ObjectPopup id={detectedObject.id} name={detectedObject.title} description={detectedObject.description} img={detectedObject.image != null ? detectedObject.image.path : null} x={detectedObject.x} y={detectedObject.y} />));
                }
                if (detectedObject.missionId != null && detectedObject.isDesired === false) {
                    return L.marker([parseFloat(detectedObject.x), parseFloat(detectedObject.y)], {
                        icon: BusyIcon,
